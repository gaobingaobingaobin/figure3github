    function  L= lagrangePi10(p_Variable,p_00Vec,p_01Vec,p_10Vec,p_11Vec...
                ,PHvec,g1Vec,g2Vec,hVec,I1,I2,lambdaVec,betaVec,lambda10Vec,beta10,PavVec,channelNum,ST)
            %I1 是主用户PT 1 的上限干扰功率
...
lambda=lambdaVec(ST,channelNum);
    beta=betaVec(ST);
    lambda10=lambda10Vec(channelNum);
%     beta10=beta10Vec(ST);
...
%     g2=g2Vec(ST,channelNum);
    ...
    tempSUM=P1H0.*P2H0.*p_00+P1H0.*P2H1.*p_01+P1H1.*P2H0.*p_10+...
           P1H1.*P2H1.*p_11;
       tempSUM2=sum(g1Vec.*p_10Vec);
    L = -P1H1.*P2H0.*log(1+h.*p_variable)-lambda.*P1H1.*P2H0.*p_variable...
        +beta./2 .* (tempSUM-P1H1.*P2H0.*p_10 +P1H1.*P2H0.*p_variable-Pav ).^2 ...
        -lambda10.*g1.*p_variable+beta10./2.* (tempSUM2(channelNum)-g1.*p_10+g1.*p_variable -I1).^2;
